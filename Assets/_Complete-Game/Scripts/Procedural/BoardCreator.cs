...
public class BoardCreator : MonoBehaviour
{
	...
	private void Start ()
	{
		...
		maxEnemies = enemyCount.Random;
		Debug.Log ("Value of maxEnemies: " + maxEnemies);
	...
	void CreateRoomsAndCorridors ()
	{
		...
		for (int i = 1; i < rooms.Length; i++)
		{
			...
			if (i == Mathf.Floor (rooms.Length * .5f) && !spawnedPlayer) {
				Vector3 playerPos = new Vector3 (rooms [i].xPos, rooms [i].yPos, 0);
				Instantiate (player, playerPos, Quaternion.identity);
				spawnedPlayer = true;
			} 

			else if (enemyCount.Random > 0.65 && maxEnemies > 0) {
	...
	/*
	void Instantiate (GameObject prefab, float xCoord, float yCoord)
	{
		// The position to be instantiated at is based on the coordinates.
		Vector3 position = new Vector3(xCoord, yCoord, 0f);
